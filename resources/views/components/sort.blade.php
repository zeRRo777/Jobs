        ...
        <form method="GET" action="{{ route($route) }}">
            ...
            <div class="relative">
                <select
                    ...
                    <option value="" disabled {{ !request('sort') ? 'selected' : '' }}>Выберите</option>
                    <option value="asc" {{ request('sort_' . $subjectSort) == 'asc' ? 'selected' : '' }}>По возрастанию</option>
                    <option value="desc" {{ request('sort_' . $subjectSort) == 'desc' ? 'selected' : '' }}>По убыванию</option>
                </select>
